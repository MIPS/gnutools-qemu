...
# define MAX_FP_ABI Val_GNU_MIPS_ABI_FP_SOFT
	...
	if (prog_req.fr1 || prog_req.frdefault || prog_req.fre) {
	    ...
	    if ((fp_abi == -1 && info->interp_fp_abi == Val_GNU_MIPS_ABI_FP_SOFT)
		|| (info->interp_fp_abi == -1 && fp_abi == Val_GNU_MIPS_ABI_FP_SOFT)) {
		fp_abi = Val_GNU_MIPS_ABI_FP_SOFT;
#ifdef TARGET_ABI_MIPSO32
            /* Need to merge the FP ABIs.  */
	    } else if (fp_abi == -1
                       && (info->interp_fp_abi == Val_GNU_MIPS_ABI_FP_DOUBLE
                           || info->interp_fp_abi == Val_GNU_MIPS_ABI_FP_XX)) {
                fp_abi = Val_GNU_MIPS_ABI_FP_DOUBLE;
	    } else if (info->interp_fp_abi == -1
                       && (fp_abi == Val_GNU_MIPS_ABI_FP_DOUBLE
                           || fp_abi == Val_GNU_MIPS_ABI_FP_XX)) {
                fp_abi = Val_GNU_MIPS_ABI_FP_DOUBLE;
	    } else if (fp_abi == Val_GNU_MIPS_ABI_FP_XX
                && (info->interp_fp_abi == Val_GNU_MIPS_ABI_FP_DOUBLE
                    || info->interp_fp_abi == Val_GNU_MIPS_ABI_FP_64A
                ...
                fp_abi = Val_GNU_MIPS_ABI_FP_64A;
#else
	    } else if ((fp_abi == -1 && info->interp_fp_abi == Val_GNU_MIPS_ABI_FP_DOUBLE)
		       || (info->interp_fp_abi == -1 && fp_abi == Val_GNU_MIPS_ABI_FP_DOUBLE)) {
		fp_abi = Val_GNU_MIPS_ABI_FP_DOUBLE;
#endif
            } else {
                fprintf(stderr, "qemu: Program and interpreter require "
