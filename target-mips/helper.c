...
*
* For readability, this pseudo-code does not deal with PTEs of different
* widths.
...
#ifdef MIPSSIM_COMPAT
#ifdef HTW_DEBUG
    sv_log("HTW: entry = %016lx, result = %016lx, PTEI = 0x%08x\n",
            entry, result, ptei);
#endif
#endif
    return result;
        ...
        uint64_t *pw_entrylo0, uint64_t *pw_entrylo1)
{
    ...
    if (get_physical_address(env, &paddr, &prot, *vaddr, MMU_DATA_LOAD,
            ACCESS_INT) != TLBRET_MATCH) {
        /* wrong base address */
#ifdef MIPSSIM_COMPAT
#ifdef HTW_DEBUG
        sv_log("HTW: wrong base address\n");
#endif
#endif
        return 0;
    }
...
#ifdef MIPSSIM_COMPAT
#ifdef HTW_DEBUG
    sv_log("HTW: directory entry: [0x%016lx] = 0x%016lx\n", *vaddr,
            entry);
#endif
#endif
        ...
        if (directory_index & 0x1) {
            ...
            *pw_entrylo1 = entry | lsb; /* odd page */
            ...
            if (get_physical_address(env, &paddr, &prot, vaddr2,
                    MMU_DATA_LOAD, ACCESS_INT) != TLBRET_MATCH) {
                return 0;
            }
...
static int page_table_walk_refill(CPUState *cs, vaddr address, int rw,
        int mmu_idx)
{
    MIPSCPU *cpu = MIPS_CPU(cs);
...
#ifdef MIPSSIM_COMPAT
#ifdef HTW_DEBUG
    sv_log("HTW: Root: Invoked at PC = 0x" TARGET_FMT_lx
           " for address 0x%016" PRIx64 " (htw is root)\n",
           env->active_tc.PC,
           address);

    sv_log("HTW: PWBase    = 0x" TARGET_FMT_lx "\n", env->CP0_PWBase);
    sv_log("     PWSize    = 0x" TARGET_FMT_lx "\n", env->CP0_PWSize);
    sv_log("     PWCtl     = 0x%08x\n", env->CP0_PWCtl);
    sv_log("     GDI = 0x%08x, GDW = 0x%08x\n", gdi, gdw);
    sv_log("     UDI = 0x%08x, UDW = 0x%08x\n", udi, udw);
    sv_log("     MDI = 0x%08x, MDW = 0x%08x\n", mdi, mdw);
    sv_log("     dirShift  = %d, leafShift = %d\n", directory_shift,
            leaf_shift);
    sv_log("     gOffset   = 0x%08x\n", goffset);
    sv_log("     uOffset   = 0x%08x\n", uoffset);
    sv_log("     mOffset   = 0x%08x\n", moffset);
    sv_log("     ptOffset0 = 0x%08x\n", ptoffset0);
    sv_log("     ptOffset1 = 0x%08x\n", ptoffset1);
#endif
#endif
    ...
    /* Temporal values for get_physical_address() */
    uint64_t paddr;
    int prot;
...
#ifdef MIPSSIM_COMPAT
#ifdef HTW_DEBUG
        sv_log("HTW: Leaf page 0 walking error. Aborting.\n");
#endif
#endif
        return 0;
    }
...
#ifdef MIPSSIM_COMPAT
#ifdef HTW_DEBUG
    sv_log("HTW: Leaf   directory entry [0]: [%08lx] = %08lx\n", vaddr,
            dir_entry);
#endif
#endif
...
#ifdef MIPSSIM_COMPAT
#ifdef HTW_DEBUG
        sv_log("HTW: Leaf page 1 walking error. Aborting.\n");
#endif
#endif
        return 0;
    }
...
#ifdef MIPSSIM_COMPAT
#ifdef HTW_DEBUG
    sv_log("HTW: Leaf   directory entry [1]: [%08lx] = %08lx\n", vaddr,
            dir_entry);
#endif
#endif
...
refill:
    ...
    pw_pagemask = m >> 12;
    update_pagemask(env, pw_pagemask << 13, (int32_t *) &pw_pagemask);
    pw_entryhi = (address & ~0x1fff) | (env->CP0_EntryHi & 0xFF);

#ifdef MIPSSIM_COMPAT
#ifdef HTW_DEBUG
    sv_log("HTW: PWMask = 0x%08x, EntryHi = 0x%08x, "
            "EntryLo0 = 0x%016lx, EntryLo1 = 0x%016lx\n",
            pw_pagemask, pw_entryhi, pw_entrylo0, pw_entrylo1);
#endif
#endif

    r4k_helper_tlbwr_common(env, pw_pagemask, pw_entryhi, pw_entrylo0,
            pw_entrylo1);
    return found;
}
#endif
...
#if !defined(CONFIG_USER_ONLY)
#if !defined(TARGET_MIPS64)
        if (ret == TLBRET_NOMATCH && env->tlb->tlb_type == MMU_TYPE_R4000) {
            /* Memory reads during hardware page table walking are performed
             * as if they were kernel-mode load instructions. */
            int mode = (env->hflags & MIPS_HFLAG_KSU);
            env->hflags &= ~MIPS_HFLAG_KSU;
            int ret_walker = page_table_walk_refill(cs, address, rw,
                    mmu_idx);
            env->hflags |= mode;
            if (ret_walker) {
                ret = get_physical_address(env, &physical, &prot,
