                               ...
                               bool search_pc)
{
    ...
    while (ctx.bstate == BS_NONE) {
        ...
        if (svtracefile) {
            ...
            TCGv opc = tcg_const_tl(cpu_ldl_code(env, ctx.pc));
            gen_helper_trace_transl_pre(cpu_env, curr_pc, opc);
        ...
        if (svtracefile) {
	  TCGv curr_pc = tcg_const_tl(ctx.pc);
            gen_helper_trace_transl_post(cpu_env, curr_pc);
        }
#endif
        ctx.pc += insn_bytes;
