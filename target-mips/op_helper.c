...
static void trace_tlb_fill(CPUMIPSState *env, int idx, bool is_random)
{
    ...
    sv_log("PFN0 0x%16" PRIx64 " ", tlb->PFN[0] << 12);
    sv_log("PFN1 0x%16" PRIx64 " ", tlb->PFN[1] << 12);
...
void helper_trace_transl_pre(CPUMIPSState *env, target_ulong trace_pc, target_ulong opc)
{
    if (use_icount) {
        /* If '-icount' is specified then set instruction number limit */
        check_inst_limit();
    }
    if (trace_pc == 0x12002e518ULL && opc == 0x67bdff20ULL)
    {
      trace_enabled = true;
    }
      trace_enabled = (((env->hflags & MIPS_HFLAG_KSU) && ((env->hflags & MIPS_HFLAG_KSU) != MIPS_HFLAG_SM))
...
void helper_trace_transl_post(CPUMIPSState *env, target_ulong trace_pc)
{
    if (trace_enabled)
    {
     trace_cpu_state(env, 0);
    }
    if (trace_pc == 0x12002fabcULL)
    {
      trace_enabled = false;
    }
}
